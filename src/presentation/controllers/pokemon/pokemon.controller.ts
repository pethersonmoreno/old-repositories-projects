...
import { NumberInteger, Pokemon, PokemonDTO, PokemonRepository, PokemonService } from 'src/domain/pokemon';
...
export class PokemonController {
    ...
    constructor(@Inject('PokemonRepository') pokemonRepository: PokemonRepository) {
    ...
    async detailPokemon(@Param() params: { id: string }, @Res() res: Response) {
        ...
        try{
            const pokemon = await this.pokemonRepository.findByPokemonId(NumberInteger.create(pokemonId));
            if(!pokemon){
    ...
    @Put(':id')
    async updatePokemon(@Body() pokemonDtoWithoutId: Omit<PokemonDTO, 'pokemonId'>, @Param() params: { id: string }, @Res() res: Response) {
        const pokemonId = parseInt(params.id, 10);
        if(isNaN(pokemonId)){
            this.sendErrorResponse(new ValidationError('Invalid Pokemon ID'), res);
            return;
