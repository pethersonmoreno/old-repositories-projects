...
import { NumberInteger, Pokemon as PokemonEntity, PokemonName, PokemonRepository, PokemonType } from "../../../domain/pokemon";
...
export default class TypeormPokemonRepository implements PokemonRepository {
    ...
    async findByPokemonId(pokemonId: NumberInteger): Promise<PokemonEntity> {
        const pokemonById = await this.pokemonRepositoryTypeorm.findOne({ where: { pokemonId: pokemonId.value }})
        if (!pokemonById){
            return undefined;
        }
    ...
    async save(pokemon: PokemonEntity): Promise<void> {
        const pokemonById = await this.pokemonRepositoryTypeorm.findOne({ where: { pokemonId: pokemon.pokemonId.value }})
        if (!pokemonById){
