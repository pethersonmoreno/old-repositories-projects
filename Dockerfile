FROM python:3.11.12-bookworm as pjsip-build
ARG PJSIP_VERSION=2.15.1
ENV SIP_ROOT_DIR=/opt/pjsip
MAINTAINER Petherson Moreno <pethermoreno@gmail.com>
...
RUN mkdir /tmp/src-build-pjsua2 && \
    ...
    export PJ_CXXFLAGS="-g -O2 -DPJ_AUTOCONF=1 -fPIC -O2 -DNDEBUG -DPJ_IS_BIG_ENDIAN=0 -DPJ_IS_LITTLE_ENDIAN=1 -fPIC -I${SIP_ROOT_DIR}/include" && \
    make && \
